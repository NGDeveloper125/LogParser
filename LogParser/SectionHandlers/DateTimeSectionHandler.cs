...
internal class DateTimeSectionHandler : ISectionHandler
{
    private static readonly string defaultPattern =
    @"^\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}:\d{2}(?:\.\d{3,6})?(?:Z|[+-]\d{2}:\d{2})?";

    public LogLineParserResult HandleSection(string logLine, int sectionIndex)
    {
        if (string.IsNullOrEmpty(logLine))
            return new LogLineParserResult(false, [], "Failed to find datetime section in log line");
